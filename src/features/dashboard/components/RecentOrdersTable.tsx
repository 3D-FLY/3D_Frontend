...
export default function RecentOrdersTable({ items }: RecentOrdersTableProps) {
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden">
      {/* Header */}
      <div className="grid grid-cols-[1fr_1.5fr_1.2fr_1fr_24px] gap-3 px-1 pb-2 border-b border-white/20">
        {["Order", "Store", "Status", "Date", ""].map((col) => (
          <span key={col} className="text-[11px] font-semibold uppercase tracking-wide text-black/40">
            {col}
          </span>
        ))}
      </div>
        ...
        {capped.map((order, index) => {
          ...
          return (
            <div
              ...
              className={`
                grid grid-cols-[1fr_1.5fr_1.2fr_1fr_24px] gap-3 items-center py-2.5 px-1
                cursor-pointer hover:bg-white/10 transition-colors rounded-md
                ${index < capped.length - 1 ? "border-b-[0.5px] border-white/50" : ""}
              `}
            >
              {/* Order */}
              <span className="text-[clamp(12px,1vw,14px)] font-semibold text-black truncate">
                #{order.orderId}
              </span>

              {/* Store */}
              <span className="text-[clamp(12px,1vw,14px)] text-black/70 truncate">
                {order.store}
              </span>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[clamp(11px,0.9vw,13px)] font-semibold uppercase tracking-wide text-black truncate">
                  {config.label}
                </span>
              </div>
